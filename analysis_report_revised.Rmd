...
output_fig_dir= 
  "C:/Users/wenyu/Documents/functional genomics/manuscript3_ebiomedicine/figdir2/"
library(data.table)
library(tidyr)
library(dplyr)
library(openxlsx)
...
export_fig <- function(fig_name, width= 10.7, height, unit="cm", plot = NULL) {
    ...
    ggsave(file.path(output_fig_dir, paste0(fig_name, '.svg')), 
             width = width, height = height, dpi=500, plot = plot)
    dev.off()
    ggsave(file.path(output_fig_dir, paste0(fig_name, '.tiff')),
           width = width, height = height, plot = plot,dpi=500)
    dev.off()
}
```
...
# mean_mat <-  data_long1 %>%
#   group_by(gene) %>%
#   summarise_at(.vars = c(17:25),.funs = mean, na.rm=T)
...
auc_table <- function(fig,ref_method= "ces"){
  check <- data.frame(
  ...
  p_auc <- function( new_method_name ){
    library(pROC)
    tmp1 = fig$data
  ...
  check %>%  tbl_df() %>% arrange(desc(AUC_value)) %>% 
    mutate(p= mapply(FUN = p_auc, score_names))
}
```
...
#fig3a2 <- fig3a2+ theme(legend.position = "bottom" )+guides(colour = guide_legend(nrow = 3, byrow = T))
fig3a2
#auc_table(fig3a2,ref_method = "ces")
```
...
fig3b2
#auc_table(fig3b2,ref_method = "ces")
```
...
fig3_1 <- ggarrange(fig3a2,fig3b2,labels=letters[1:2],legend = "none")
#fig3_1 
...
tmp <-
  ...
  mutate(cell_line_ave_demeter2= mean(demeter,na.rm = T)) %>%
...
### pancancer oncogenes and tsg, figs5b and figs6c
```{r,fig.width=8,fig.height=8.5}
plot_den <- function(cell_name, method){
 tmp1 <-  tmp %>%
