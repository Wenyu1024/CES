...
output_fig_dir= 
  "C:/Users/wenyu/Documents/functional genomics/manuscript3_ebiomedicine/fig_dir/checkworkflow"
...
data_long_null <- fread("data_long1_new.csv") #data is not normalized
...
apply(data_long_null[,2:9], 2, Hmisc::describe)


#first use cell-wise z normalization
data_long <- data_long_null %>% 
  group_by(cell) %>% 
  mutate_at(.vars = 2:9,.funs = scale) %>% 
  ungroup(cell)
summary(data_long)
apply(data_long[,2:9], 2, var, na.rm=T)
```
...
hk_gene <- hk_gene$V1
comess_gene <- fread(file = "C:/Users/wenyu/Documents/functional genomics/manuscript3_ebiomedicine/data_and_code/training_essentials.txt",header = T)
#https://www.g3journal.org/content/7/8/2719
comess_gene <- comess_gene$Gene
#http://msb.embopress.org/content/10/7/733.long#DC
comness_gene <- fread(file = "C:/Users/wenyu/Documents/functional genomics/manuscript3_ebiomedicine/data_and_code/NEGv1.txt",header =T)
comness_gene <- comness_gene$Gene
pancan_gene <-  scan("pancancergenes.txt",what = "character")
```
...
export_fig <- function(fig_name, width= 11.1, height, unit="cm", plot = NULL) {
    ...
    # dev.off()
    ggsave(file.path(output_fig_dir, paste0(fig_name, '.png')), 
           width = width, height = height, plot = plot)
    dev.off()
}
```
...
what is overlap of the four sets
```{r}

con1 <- intersect(comess_gene,comness_gene)
con2 <- intersect(hk_gene,comness_gene)
con3 <- intersect(pancan_gene,comness_gene)
con <- c(con1,con2,con3)
comess_gene <- setdiff(comess_gene,con)
hk_gene <- setdiff(hk_gene,con)
pancan_gene <- setdiff(pancan_gene,con)
comness_gene <- setdiff(comness_gene,con)
cancergene <- unique(c(hk_gene,pancan_gene,comess_gene))
c3 <- list(A=hk_gene,B=comess_gene,C= pancan_gene )
require(VennDiagram)
figs1 <- venn.diagram(x = c3,
                      filename = NULL, 
                      category.names =c("Housekeeping\ngenes", "Common essential\ngenes", "PanCancer genes"),
                      fill=rainbow(3),
                      resolution = 350,
                     height = 11,width = 11.1,
                     cex= 3,
                     cat.cex=c(3.5,3.5,3.5),
                     margin=0.15,
                      cat.default.pos="outer"
                        )
export_fig("figs1",height = 11,width = 11,plot =figs1)
...
tmp <- data_long %>% 
  ...
  mutate(pancancer_gene = gene %in% pancan_gene) %>% 
...
### fig s2b: pancan genes 
```{r,fig.height=5, fig.width=5}
tmp1 <- tmp %>% 
  mutate(gene_type= 
           case_when(pancancer_gene  ~ "ess",
                    TRUE ~ "others" ) )%>%
  filter(!(is.na(gene_type))) 
...
figs2b <- tmp1 %>% 
  ...
  scale_x_discrete( labels= c("CRISPR", "shRNA"))+
  scale_fill_manual(name=element_blank(),
                    labels = c("PanCancer genes", "Others"),
                    values=c("red", "green") )+
  ylab("Essentiality score\n")+
  theme(plot.margin=unit(c(0,1,1,1),"cm"))+
  guides(fill=guide_legend( nrow = 2, byrow = T))
...
export_fig("fig2",height= 11, plot =  fig2)
fig2
...
figs2
export_fig("figs2",height= 11, plot =  figs2)
```
...
data_long1 <- left_join(x = data_long, y = tmp2) %>%   
  ...
  group_by(cell) %>% 
  mutate_at(.vars = 12:15,.funs = scale) %>% 
  ungroup()


## 
apply(data_long1[,c(2:9,12:15)], 2, var,na.rm=T)
...
```{r}
tmp = plyr::ddply(data_long1,"gene", summarise,
...
#   data_long1 %>% 
#   group_by(gene) %>% 
#   summarise_at(.vars = 16:23,.funs = mean, na.rm=T)
```
...
tmp1 <- tmp %>% filter(gene %in% c(hk_gene, comness_gene)) %>% 
...
ROC plot for common essential genes
```{r, fig.width=5.5}
# ROC plot for common essential genes
tmp1 <- tmp %>% filter(gene %in% c(comess_gene, comness_gene)) %>% 
...
ROC plot for pancancer genes
```{r, fig.width=7.5,fig.height=5.5}

tmp1 <- tmp %>% filter(gene %in% c(pancan_gene, comness_gene)) %>% 
  mutate(group= case_when(gene %in% pancan_gene~ 0, TRUE ~1))
...
figs3 = longtest %>% 
  ...
  theme(
    ...
    scale_color_manual(values = pal,name = "Method", 
                         labels = c("SA","CERES","ces_null_name","ces_perm_name","CES", "CRISPR", "DEMETER", "shRNA" ))

figs3 <- figs3+ theme(legend.position = "bottom" )
calc_auc(figs3)$AUC
```
...
no cell level scaling
```{r}
check <- data.frame(
  cell= rep(c("a","b"),20), 
  gene=sort(rep(c("pos","neg"),20))) %>% 
  tbl_df() %>% 
  rowwise() %>%
  mutate(score1= case_when((cell=="a")& (gene=="pos")~ rnorm(n = 1,mean = -6,sd = 0.5),
                           (cell=="b")& (gene=="pos")~ rnorm(n = 1,mean = -4,sd = 1),
                           (cell=="a")& (gene=="neg")~ rnorm(n = 1,mean = 0,sd = 0.5),
                           (cell=="b")& (gene=="neg")~ rnorm(n = 1,mean = 0,sd = 1)),
         score2= case_when((cell=="a")& (gene=="pos")~ rnorm(n = 1,mean = -2,sd = 0.5),
                           (cell=="b")& (gene=="pos")~ rnorm(n = 1,mean = -2,sd = 1),
                           (cell=="a")& (gene=="neg")~ rnorm(n = 1,mean = 0,sd = 0.5),
                           (cell=="b")& (gene=="neg")~ rnorm(n = 1,mean = -0,sd = 1))
  ) %>% 
  ungroup() %>% 
  group_by(cell) %>% 
  mutate(score1_scaled=scale(score1),
         score2_scaled=scale(score2))
  
check %>% ungroup() %>% 
  select(1:4) %>% 
  gather(key = "method",value = "score",3:4) %>% 
  ggplot(aes(x=method,
             y=score,
             color=gene))+
  geom_boxplot()+
...
get_ssmd_from_data_long1 <- function(x){
  SSMD_pos= 
    check %>% 
    filter(gene=="pos") %>% 
    summarise(sample_mean_1= mean(!!rlang::sym(x),na.rm = T),  
              sample_sd_1= sd(!!rlang::sym(x),na.rm =T)  ) 
  
  SSMD_neg= 
    check %>% 
    filter(gene=="neg") %>% 
    summarise(sample_mean_2= mean(!!rlang::sym(x),na.rm = T),  
              sample_sd_2= sd(!!rlang::sym(x),na.rm =T)  ) 
...
plot_ssmd_df <- 
  data.frame(
    ssmd1=get_ssmd_from_data_long1("score1_scaled"),
    ssmd2=get_ssmd_from_data_long1("score1")
    ) %>% 
  tbl_df() %>% 
  mutate(rank1= rank(ssmd1)) %>% 
  gather(key= method, value = ssmd, ssmd1:ssmd2)

ggplot(plot_ssmd_df,mapping = aes(y= ssmd, x = rank1, col= method)) +
  geom_point()+
  theme_classic()
...
# data_long1   %>%
#   group_by(cell) %>% 
#   mutate(ces1 = scale(ces)) %>% 
#   ungroup() %>% 
#   filter(gene %in% hk_gene) %>% 
#   group_by(cell) %>% 
#   summarise(
#     var1=var(ces,na.rm = T),
#     var2=var(ces1,na.rm = T)
#     )
...
# get_ssmd_from_data_long1(x = "ces")
# get_ssmd_from_data_long1(x = "ces1")
...
fig3f <- ggplot(plot_ssmd_df,
  ...
  theme(plot.margin=unit(c(0.5,0.5,0.5,0.5),"cm"))+
  scale_x_discrete(labels=c("CES", "CERES", "SA",ces_null_name, "shRNA",ces_perm_name,"CRISPR", "DEMETER") )
...
t.test(ssmd_ces, ssmd_ceres,paired = T)
(mean(ssmd_ces,na.rm = T) -mean(ssmd_ceres,na.rm = T))/mean(ssmd_ceres,na.rm = T)
...
figs4a <- ggplot(plot_ssmd_df,mapping = aes(y= ssmd, x = rank.ssmd_ces., col= method)) +
  ...
  xlab("Cell lines rank")+
  labs(col= " Method")+
  scale_color_manual(values = pal,labels= c("SA","CERES",ces_null_name,ces_perm_name,"CES", "CRISPR", "DEMETER", "shRNA" ))+
  theme(plot.margin=unit(c(0.5,0.5,0.5,0.5),"cm"))
...
```{r,fig.width=5.55}
### pancan_gene based
tmp1= data_long1   %>%filter(gene %in% pancan_gene) %>% group_by(cell) 
tmp2= data_long1   %>%filter(gene %in% comness_gene) %>% group_by(cell) 
...
fig3
export_fig("fig3",height= 11, width = 9.5, plot =  fig3)

export_fig("figs3",height= 6, width = 6, plot =  figs3)
...
figs4
export_fig("figs4",height= 8,  plot =  figs4)
...
## figure 4 and supplementary figure s5-6
...
tmp <-
  ...
  mutate(true_essential_gene = gene %in% comess_gene) %>%
  mutate(non_essential_gene = gene %in% comness_gene) %>%
  mutate(pancan_gene = gene %in% pancan_gene) 
...
fig4.1 <- plot_den("PC3","crispr") +ggtitle("CRISPR")
fig4.2 <- plot_den("PC3","rnai_un")+ ggtitle("shRNA")
fig4.3 <- plot_den("PC3","ces")+ ggtitle("CES")
fig4.4 <- plot_den("PC3","demeter")+ggtitle("DEMETER")
fig4.5 <- plot_den("PC3","ceres")+ggtitle("CERES")
fig4.6 <- plot_den("PC3","simple_ave")+ggtitle("SA")
fig4.7 <- plot_den("PC3","ces_null")+ggtitle(ces_null_name)
fig4.8 <- plot_den("PC3","ces_perm")+ggtitle(ces_perm_name)
...
figs6a5 <- plot_den("PC3","ceres")+ggtitle("CERES")
figs6a6 <- plot_den("PC3","simple_ave")+ggtitle("SA")
figs6a7 <- plot_den("PC3","ces_null")+ggtitle(ces_null_name)
figs6a8 <- plot_den("PC3","ces_perm")+ggtitle(ces_perm_name)
figs6a <- ggarrange(figs6a3,figs6a7,figs6a8,figs6a6, figs6a5,figs6a1,figs6a4,figs6a2, ncol = 4, nrow = 2,
                  common.legend = T,
                  legend = "bottom")
figs6a
...
figs5a5 <- plot_den("HT29","ceres")+ggtitle("CERES")
figs5a6 <- plot_den("HT29","simple_ave")+ggtitle("SA")
figs5a7 <- plot_den("HT29","ces_null")+ggtitle(ces_null_name)
figs5a8 <- plot_den("HT29","ces_perm")+ggtitle(ces_perm_name)
figs5a <- ggarrange(figs5a3,figs5a7,figs5a8,figs5a6, figs5a5,figs5a1,figs5a4,figs5a2, ncol = 4, nrow = 2,
                  common.legend = T,
                  legend = "bottom")
figs5a
...
figs6b5 <- plot_den("PC3","ceres")+ggtitle("CERES")
figs6b6 <- plot_den("PC3","simple_ave")+ggtitle("SA")
figs6b7 <- plot_den("PC3","ces_null")+ggtitle(ces_null_name)
figs6b8 <- plot_den("PC3","ces_perm")+ggtitle(ces_perm_name)
figs6b <- ggarrange(figs6b3,figs6b7,figs6b8,figs6b6, figs6b5,figs6b1,figs6b4,figs6b2, ncol = 4, nrow = 2,
                  common.legend = T,
                  legend = "bottom")
figs6b
```

### pancancer genes, figs5b and figs6c
```{r,fig.width=11.1,fig.height=6}
plot_den <- function(cell_name, method){
 tmp1 <-  tmp %>%
    mutate(essential=  case_when(pancan_gene ~ "True essential",
                                 non_essential_gene ~ "Non-essential" )) %>% 
    filter(cell ==  cell_name) 
    ...
    ylab("Cell line essentiality score")+
    guides(color=guide_legend(title="Gene type"))+
    scale_color_manual(values = pal,labels= c("Common non-essential genes   ", "  PanCancer genes"))+
    theme(plot.title = element_text(hjust = 0.5,colour = "black",size = "18"))+
    theme(plot.margin=unit(c(0.5,0.5,0.5,0.5),"cm"))+
    guides(fill=guide_legend( nrow = 2, byrow = F))  
...
figs5b5 <- plot_den("HT29","ceres")+ggtitle("CERES")
figs5b6 <- plot_den("HT29","simple_ave")+ggtitle("SA")
figs5b7 <- plot_den("HT29","ces_null")+ggtitle(ces_null_name)
figs5b8 <- plot_den("HT29","ces_perm")+ggtitle(ces_perm_name)
figs5b <- ggarrange(figs5b3,figs5b7,figs5b8,figs5b6, figs5b5,figs5b1,figs5b4,figs5b2, ncol = 4, nrow = 2,
                  common.legend = T,
                  legend = "bottom")
figs5b
...
figs6c5 <- plot_den("PC3","ceres")+ggtitle("CERES")
figs6c6 <- plot_den("PC3","simple_ave")+ggtitle("SA")
figs6c7 <- plot_den("PC3","ces_null")+ggtitle(ces_null_name)
figs6c8 <- plot_den("PC3","ces_perm")+ggtitle(ces_perm_name)
figs6c <- ggarrange(figs6c3,figs6c7,figs6c8,figs6c6, figs6c5,figs6c1,figs6c4,figs6c2, ncol = 4, nrow = 2,
                  common.legend = T,
                  legend = "bottom")
figs6c
```
...
export_fig("figs5", height = 12,plot = figs5)
figs6 <- ggarrange(figs6a,figs6b,figs6c,nrow = 3, labels=c("a","b","c"))
export_fig("figs6", height = 18,plot = figs6)
```
...
```{r}
#output a table for GSEA analysis
...
tmp1 <- 
  ...
  summarise(avevalue= mean(cn, na.rm=T),
            averank= mean(ces_rank, na.rm=T)) %>%
  filter(gene %in% c(pancan_gene,comness_gene)) %>% 
  mutate(true_ess= case_when(gene %in% pancan_gene ~ "True essential", 
                             gene %in% comness_gene ~ "Non-essential")) %>% 
  mutate(true_ess=as.factor(true_ess))
...
figs8 <- 
  ...
  ylab("CES rank")+
  scale_x_discrete( labels=seq(5,95,10))+
  scale_fill_manual(labels= c("Common non-essential genes", "PanCancer genes"),values = pal)+
  guides(fill=guide_legend(nrow = 2, byrow = F))
...
fig6
export_fig(fig_name = "fig6",plot = fig6,height = 11)  


export_fig(fig_name = "figs8", plot=figs8,height = 5.5,width = 5.5)
```
...
one_coef_genes <- 
  ...
  filter(coef_type!="(Intercept)") %>% 
  group_by(gene) %>% 
  summarise(num_of_coef=n()) %>% 
  filter(num_of_coef == 1) %>% 
  pull(gene)

mut_only_gene <- 
  coef_df %>% 
  filter(P< 0.05) %>% 
  filter(gene %in% one_coef_genes) %>% 
  filter(coef_type == "mut") %>% 
  pull(gene)
  
#check  NRAS, MYC
data_long1 %>% filter(gene == "NRAS" ) 
...
s9.1 <- ggplot(data= tmp2, mapping = aes(reorder(cell,array), y = scale(array),fill= ess_cell)) + 
...
##
s9.2 <- ggplot(data= tmp2, mapping = aes(x=reorder(cell, seq), y = scale(seq),fill= ess_cell)) +
  geom_bar(stat = "identity")+
   theme_classic()+
  theme(axis.text.x = element_text(size = 12,angle = 0, hjust = 1,color = "black"))+
  theme(axis.text.y = element_text(size = 10,colour = "black"))+
...
figs9 <- ggarrange(s9.1,s9.2,ncol = 2,nrow = 1,common.legend = T)
figs9
export_fig(fig_name ="figs9", height = 11,plot = figs9)
